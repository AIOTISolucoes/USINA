...
def _lambda_handler_impl(event, context):
            ...
            cur.execute(sql.SQL("""
                WITH pivot AS (
                  SELECT
                    r.power_plant_id,
                    r.device_id,
                    COALESCE(MAX(r.device_name), MAX(d.name), MAX(d.display_name)) AS inverter_name,
    ...
    if method == "POST" and is_path(path, "/users/notif-prefs"):
        ...
        if not isinstance(prefs, dict):
            return http_response(400, {"error": "prefs deve ser um objeto"})
